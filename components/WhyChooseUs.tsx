        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {academics.map((feature, idx) => (
            <ScrollReveal key={idx} direction="up" delay={idx * 0.08}>
              <div className="relative p-[1px] rounded-[2rem] overflow-hidden group transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(201,150,43,0.15)] h-full">
                ...
                {/* Inner Card content */}
                <div className="relative h-full bg-navy-dark rounded-[2rem] p-8 sm:p-10 flex flex-col overflow-hidden">
                  ...
                  {/* Index Number */}
                  <div className="absolute top-6 right-8 font-[family-name:var(--font-heading)] text-5xl text-white/[0.03] font-black group-hover:text-gold/10 transition-colors duration-500 select-none">
                    0{idx + 1}
                  </div>

                  {/* Icon */}
                  <div className="w-16 h-16 rounded-2xl bg-white/[0.03] border border-white/10 flex items-center justify-center text-gold mb-8 group-hover:scale-110 group-hover:border-gold/40 group-hover:bg-gold/10 transition-all duration-500 relative z-10 shadow-[inset_0_0_20px_rgba(255,255,255,0.02)] group-hover:shadow-[inset_0_0_20px_rgba(201,150,43,0.2)]">
                    {feature.icon}
                  </div>
                  
                  <h3 className="text-2xl font-bold text-white font-[family-name:var(--font-heading)] mb-4 transition-colors relative z-10 group-hover:text-gold">
                    {feature.title}
                  </h3>
                  
                  <p className="text-white/60 text-[15px] font-[family-name:var(--font-body)] leading-relaxed relative z-10 group-hover:text-white/80 transition-colors mb-4 flex-1">
                    {feature.description}
                  </p>

                  {/* Hover interactive element */}
                  <div className="mt-8 pt-6 border-t border-white/5 flex items-center gap-4 group-hover:border-gold/20 transition-colors duration-500 relative z-10">
                    <span className="text-[11px] uppercase tracking-[0.2em] text-white/30 font-bold group-hover:text-gold transition-colors">Explore</span>
